...
import { Base, type IMarket } from "@woof-software/comet-sdk";
...
describe("Base Methods", () => {
  ...
  test("should contain baseMinForRewards", () => {
    expect(base.baseMinForRewards, "baseMinForRewards is missing").to.be.a(
      "bigint",
    );
    expect(
      base.baseMinForRewards,
      "baseMinForRewards is not valid",
    ).greaterThan(0);
  });

  test("should contain baseTrackingBorrowSpeed", () => {
    expect(
      base.baseTrackingBorrowSpeed,
      "baseTrackingBorrowSpeed is missing",
    ).to.be.a("bigint");
  });

  test("should contain baseTrackingSupplySpeed", () => {
    expect(
      base.baseTrackingSupplySpeed,
      "baseTrackingSupplySpeed is missing",
    ).to.be.a("bigint");
  });
