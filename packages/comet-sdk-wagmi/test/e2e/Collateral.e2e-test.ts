...
import { Collateral, IMarket} from "@woof-software/comet-sdk";
...
describe("Collateral Methods", () => {
    ...
    test("should contain collateralFactor", () => {
        ...
        expect(collateral.collateralFactor, "collateralFactor is missing or undefined").to.be.a("bigint");
        expect(collateral.collateralFactor, "collateralFactor must be greater than 0").greaterThan(0);
    });

    test("should contain totalSupplyAssetUSD", () => {
        const collateral = collaterals[0]!;
        expect(collateral.totalSupplyAssetUSD, "totalSupplyAssetUSD must be greater than 0").greaterThan(0);
    });

    test("should contain collateralFactor", () => {
        const collateral = collaterals[0]!;
        expect(collateral.supplyCapUSD, "getSupplyCapUSD must be greater than 0").greaterThan(0);
    });
});