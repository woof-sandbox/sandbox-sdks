...
export async function fetchUserMarkets(
    ...
    const marketInChain = Object.entries(marketConfig).flatMap(async ([chainId, marketsComets]) => {
        ...
        const cometsBaseData = await multicall(wagmiConfig, {
            ...
            contracts: marketsComets.flatMap((cometProxyAddress) => {
                const comet = new CometContract(cometProxyAddress, Number(chainId) as WagmiChainId);
        ...
        return Promise.all(marketsComets.map(async (cometProxyAddress, index) => {
            const currentCometBaseData = cometsBaseData.slice(index * 7, 7 * (index + 1));


            const baseTokenAddress = WagmiUtils.resultOrThrow<Address>(currentCometBaseData[0]);
            const baseTokenPriceFeed = WagmiUtils.resultOrThrow<Address>(
                currentCometBaseData[1],
            );
            const utilization = WagmiUtils.resultOrThrow<bigint>(currentCometBaseData[2]);
            const supplyBalance = WagmiUtils.resultOrThrow<bigint>(currentCometBaseData[3]);
            const borrowBalance = WagmiUtils.resultOrThrow<bigint>(currentCometBaseData[4]);
            const totalReserves = WagmiUtils.resultOrThrow<bigint>(currentCometBaseData[6]);

            const comet = new CometContract(cometProxyAddress, Number(chainId) as WagmiChainId);
            const baseTokenContract = new Erc20Contract(baseTokenAddress, Number(chainId) as WagmiChainId);
            ...
            const baseToken = await fetchBase(cometProxyAddress, Number(chainId) as WagmiChainId);
