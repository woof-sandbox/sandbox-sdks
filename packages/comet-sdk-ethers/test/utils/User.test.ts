...
describe('fetchUserActiveMarkets', () => {
    ...
    it('should handle pagination correctly when data spans multiple pages', async () => {
        const mockUsersPage1 = Array.from({ length: SUBGRAPH_PAGE_SIZE }).map((_, i) => ({
        ...
        const expectedLends = mockUsersPage1.filter(u => BigInt(u.principal) > 0).map(u => u.proxyCometAddress);
