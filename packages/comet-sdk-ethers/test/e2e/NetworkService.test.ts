...
import {
  ...
  InitialEndpoint,
  FallbackRPCConfig,
} from "../../src/services/network/network.service";
...
describe("NetworkService", () => {
  ...
  it("should initialize with correct primary endpoints", () => {
    const primaryJsonRpc = networkService.getCurrentRPC(1, ProviderType.JsonRpc);
  ...
  it("should add fallback endpoints", () => {
    const fallbackRPCs: FallbackRPCConfig[] = [
      { rpcUrl: "https://fallback1.rpc.url", chainId: 1, providerType: ProviderType.JsonRpc },
      { rpcUrl: "https://fallback2.rpc.url", chainId: 1, providerType: ProviderType.JsonRpc },
    ];
  ...
  it("should rotate to next endpoint on handleRPCFailure", async () => {
    ...
    networkService.addFallbackRPC(fallbackRPCs);
    expect(networkService.getCurrentRPC(1, ProviderType.JsonRpc)).toBe("https://mainnet.infura.io/v3/test");
    await networkService.handleRPCFailure(1, ProviderType.JsonRpc);
    expect(networkService.getCurrentRPC(1, ProviderType.JsonRpc)).toBe("https://fallback1.rpc.url");
  });

  it("should reset to default RPC using resetToDefaultRPC", () => {
    networkService.setRPC("https://custom.rpc.url", 1, ProviderType.JsonRpc);
    expect(networkService.getCurrentRPC(1, ProviderType.JsonRpc)).toBe("https://custom.rpc.url");
  ...
  it("should notify subscribers on RPC change", async () => {
    ...
    networkService.setRPC("https://custom.rpc.url", 1, ProviderType.JsonRpc);
    expect(callback).toHaveBeenCalledWith("https://custom.rpc.url", 1, ProviderType.JsonRpc);
    ...
    networkService.addFallbackRPC(fallbackRPCs);
    await networkService.handleRPCFailure(1, ProviderType.JsonRpc);
    expect(callback).toHaveBeenCalledWith("https://fallback1.rpc.url", 1, ProviderType.JsonRpc);
  });
});
