...
const mockCustomCollateral = ({
  tokenAddress = mockAddress1,
  inputAmount = "0.5",
} = {}) => ({ tokenAddress, inputAmount });
const mockAllowance = ({
  tokenAddress = mockAddress1,
  inputAmount = "1",
  allowance = DataUtils.toBigNumber("1", 18),
} = {}) => ({ tokenAddress, inputAmount, allowance });
const mockCall = ({ tokenAddress = mockAddress1, inputAmount = "1" } = {}) => ({
  tokenAddress,
  inputAmount,
});
const mockBaseToken = {
...
describe("UserMarketMethods", () => {
  it("borrowBalanceUsd: counts correctly $", () => {
    ...
    expect(
      UserMarketMethods.borrowBalanceUsd(borrow, BigInt(6), "1.5"),
    ).toBeCloseTo(3);
  });
  ...
  it("supplyBalanceUsd: counts correctly $", () => {
    ...
    expect(
      UserMarketMethods.supplyBalanceUsd(supply, BigInt(6), "2"),
    ).toBeCloseTo(6);
  });
  ...
  it("borrowCollateralValueUSD: counts the sum of all collaterals", () => {
    const collaterals = [
      mockCollateral(),
      mockCollateral({
        tokenAddress: mockAddress2,
        symbol: "USDC",
        price: "1",
      }),
    ];
    expect(
      UserMarketMethods.borrowCollateralValueUSD(collaterals, "1"),
    ).toBeGreaterThan(0);
  });
  ...
  it("borrowCollateralValueCustomUsd: honors customCollaterals", () => {
    ...
    expect(
      UserMarketMethods.borrowCollateralValueCustomUsd(
        collaterals,
        customCollaterals,
        "1",
      ),
  ...
  it("borrowCapacityMarketUsd: counts the limit across all collaterals", () => {
    const collaterals = [mockCollateral()];
    expect(
      UserMarketMethods.borrowCapacityMarketUsd(collaterals, "1"),
    ).toBeGreaterThan(0);
  });
  ...
  it("borrowCapacityMarketCustomUsd: honors customCollaterals", () => {
    ...
    const customCollaterals = [mockCustomCollateral({ inputAmount: "2" })];
    expect(
      UserMarketMethods.borrowCapacityMarketCustomUsd(
        collaterals,
        customCollaterals,
        "1",
      ),
  ...
  it("findMarketCollateralByAddress: finds by address", () => {
    const collaterals = [
      mockCollateral({ tokenAddress: mockAddress1 }),
      mockCollateral({ tokenAddress: mockAddress2 }),
    ];
    expect(
  ...
  it("isSomeTokenAllowanceTooSmall: true if allowance is less than inputAmount", () => {
    ...
    expect(
      UserMarketMethods.isSomeTokenAllowanceTooSmall(collaterals, allowances),
    ).toBe(true);
  });
  ...
  it("isSomeTokenAllowanceTooSmall: false if the allowance is sufficient", () => {
    ...
    const allowances = [
      mockAllowance({
        inputAmount: "1",
        allowance: DataUtils.toBigNumber("2", 18),
      }),
    ];
    expect(
      UserMarketMethods.isSomeTokenAllowanceTooSmall(collaterals, allowances),
  ...
  it("isSomeTokenAllowanceTooSmall: throws error if collateral is not found", () => {
    ...
    expect(() =>
      UserMarketMethods.isSomeTokenAllowanceTooSmall(collaterals, allowances),
    ).toThrow(MISSING_COLLATERAL_DATA().message);
  });
  ...
  it("isAllCollateralsFromMarket: true if everything is available", () => {
    ...
    const calls = [
      mockCall({ tokenAddress: mockAddress1 }),
      mockCall({ tokenAddress: mockAddress2 }),
    ];
    expect(
      UserMarketMethods.isAllCollateralsFromMarket(collaterals, calls),
    ).toBe(true);
  });
  ...
  it("availableToBorrow: considers available for borrowing", () => {
    ...
    expect(
      Number(UserMarketMethods.availableToBorrow(collaterals, "1", borrow)),
    ).toBeGreaterThanOrEqual(0);
  });

  it("netBorrowAprsCustom: delegates to MarketMethods.netBorrowAprs", () => {
    const result = UserMarketMethods.netBorrowAprsCustom(
      "1",
      mockBaseToken,
      1n,
      mockCompToken,
  ...
  it("netEarnAprsCustom: delegates to MarketMethods.netEarnAprs", () => {
    const result = UserMarketMethods.netEarnAprsCustom(
      "2",
      mockBaseToken,
      2n,
      mockCompToken,
