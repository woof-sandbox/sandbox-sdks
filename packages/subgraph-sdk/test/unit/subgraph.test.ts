...
import { CollectionCallback, CollectionCallbackParams } from '../../src';
...
describe('fetchCollection', () => {
  ...
  it('must successfully execute the query and call a callback with the result', async () => {
    const mockQuery = vi.fn((skip, limit) => `{ items(skip: ${skip}, first: ${limit}) { id name } }`);
    ...
    expect(mockCallback).toHaveBeenCalledWith(mockResponse, expect.objectContaining({
  ...
  it('must perform multiple queries for pagination', async () => {
    ...
    const mockCallback: CollectionCallback = (result, params: CollectionCallbackParams) => {
      callCount++;
      if (callCount >= 2) {
        params.loopFlag = false;
      }
    ...
    expect(mockQuery).toHaveBeenNthCalledWith(2, SUBGRAPH_PAGE_SIZE, SUBGRAPH_PAGE_SIZE);
  ...
  it('should throw an error if the answer is not ok', async () => {
    ...
    await expect(fetchCollection(mockQuery, mockCallback, mockSubgraphUrl))
  ...
  it('should throw an error if there are errors in the subgraph response', async () => {
    ...
    await expect(fetchCollection(mockQuery, mockCallback, mockSubgraphUrl))
        .rejects.toThrow('Subgraph errors: Field "unknown" is not defined Syntax error');
